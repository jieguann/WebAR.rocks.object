/*
Coffee Animation - Modified Version
Dependancies:
- THREE.JS (>v95)
- Tween.js
- TeapotBufferGeometry.js

Modified to keep the teapot visible after animation completes
*/
...
const CoffeeAnimation = (function(){
  ...
  const _settings = {
    ...
    // Keep the object visible after animation completes
    keepVisibleAfterAnimation: true
  };
  ...
  const _states = {
    ...
    potFlowing: 2,
    potFlowingEnd: 3,
    // New state for completed animation with visible object
    potCompleted: 4
  };
    ...
    start: function(onComplete){
      ...
      start_teapotRotation(function(){
        ...
        _timers.push(
          ...
          setTimeout(function(){
            // Only hide if we don't want to keep it visible
            if (!_settings.keepVisibleAfterAnimation) {
              _threeObjects.main.visible = false;
              _state = _states.idle;
            } else {
              // Keep visible but stop particles
              _threeObjects.particles.visible = false;
              _state = _states.potCompleted;
            }
